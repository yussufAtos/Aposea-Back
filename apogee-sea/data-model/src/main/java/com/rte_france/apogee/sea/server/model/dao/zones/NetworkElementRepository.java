// CHECKSTYLE:OFF
package com.rte_france.apogee.sea.server.model.dao.zones;

import com.rte_france.apogee.sea.server.model.zones.NetworkElement;
import com.rte_france.apogee.sea.server.repo.ExtendedRepository;
import org.springframework.stereotype.Repository;

import java.util.Optional;

@Repository
public interface NetworkElementRepository extends ExtendedRepository<NetworkElement, String> {
    public Optional<NetworkElement> findByObjectid(String objectid);
}
